...
def get_historic(request):
    ...
    stock_marketwatch = pd.read_csv("C://upstox_oauth//media//UpstoxList_marketwatch.csv")
    stocklist = pd.read_csv("C://upstox_oauth//media//Upstox_stocklist.csv")

    print("Hello World 2")
    for i in range(len(stock_marketwatch)):
        try:

            u.get_master_contract(stocklist.loc[stocklist['ContractSymbol'] == stock_marketwatch['ContractSymbol'].iloc[i], 'ExchangeSymbol'].iloc[0])
            y = datetime.now() - timedelta(1)
            yesterday = datetime.strftime(y, '%Y-%m-%d')
            s = y - timedelta(days=55)
            startDate = datetime.strftime(s, '%Y-%m-%d')

            x = datetime.strptime(startDate, '%Y-%m-%d')
            df = pd.DataFrame()
            while x <= datetime.strptime(yesterday, '%Y-%m-%d'):
                data = u.get_ohlc(u.get_instrument_by_symbol(stocklist.loc[stocklist['ContractSymbol'] == stock_marketwatch['ContractSymbol'].iloc[i],'ExchangeSymbol'].iloc[0], stock_marketwatch['ContractSymbol'].iloc[i]), str(int(stocklist.loc[stocklist['ContractSymbol'] == stock_marketwatch['ContractSymbol'].iloc[i],'Timeframe'].iloc[0]))+'MINUTE',
            ...
            df["34EMA"] = talib.EMA(df["close"], 34)
            df["55EMA"] = talib.EMA(df["close"], 55)

            df["UpperBand"] = upper
            df["LowerBand"] = lower

            df.to_csv('C:\\upstox_oauth\\media\\Stock_Data\\'+stock_marketwatch['ContractSymbol'].iloc[i]+'.csv')
            time.sleep(10)
        except:
            pass
    ...
    return HttpResponse("Get Historical Data")
...
def get_started(request):
    ...
    for i in range(len(stocklist_marketwatch)):
        try:
            u.get_master_contract(stocklist_marketwatch['ExchangeSymbol'].iloc[i])
        except:
            pass
    ...
    def event_handler_quote_update(message):
        global dict_out, dict_out_backup, stocklist_marketwatch, stocklist, df_signal,  bar_open, bar_high, bar_low, bar_close, bar_written, trade_happen, dfdatabase, upper, lower, middle, i

        time = datetime.now(pytz.timezone(tz)).strftime("%H:%M:%S")
        if "09:15:00" <= time < "23:00:00":
            ...
            print("Timeframe = {}".format(int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'Timeframe'].iloc[0])))
            if(int(str(time).split(':')[1]) % int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'Timeframe'].iloc[0]) == 0 and \
              bar_written[message['symbol']] == 0 and time > "09:16:00"):
                ...
                upper, middle, lower = talib.BBANDS(stock_data["close"], timeperiod=14, nbdevup=2, nbdevdn=2, matype=talib.MA_Type.SMA)
                stock_data['RSI'] = talib.RSI(stock_data["close"], timeperiod=14)
                stock_data['34EMA'] = talib.EMA(stock_data["close"], 34)
                stock_data['55EMA'] = talib.EMA(stock_data["close"], 55)
                ...
                df_stock = pd.read_csv('C:\\upstox_oauth\\media\\Stock_Data\\'+message['symbol']+'.csv')

                i = len(df_stock) - 1
                if (df_stock["RSI"].iloc[i] > 70) and ((df_stock["34EMA"].iloc[i - 1] > df_stock["55EMA"].iloc[i - 1]) and (df_stock["close"].iloc[i - 1] > df_stock['UpperBand'].iloc[i - 1])) and ((df_stock["close"].iloc[i - 1] > df_stock['34EMA'].iloc[i - 1]) and (df_stock["close"].iloc[i - 1] > df_stock['55EMA'].iloc[i - 1])) and (df_stock["high"].iloc[i] > df_stock["high"].iloc[i - 1]) and trade_happen[message['symbol']] == 0 and not(str(message['symbol']) in str(dfdatabase['Symbol'])):
                    ...
                    df1 = pd.DataFrame([[message['symbol'], datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y %H:%M:%S"), "LONG", message['ltp'], nu_of_lot, Quantity]], columns=["Symbol", "Date", "Signal", "Price", "Lots", "Qty"])
                    trade_happen[message['symbol']] = 1
                ...
                elif (df_stock["RSI"].iloc[i] < 30) and ((df_stock["34EMA"].iloc[i - 1] < df_stock["55EMA"].iloc[i - 1]) and (df_stock["close"].iloc[i - 1] < df_stock['LowerBand'].iloc[i - 1])) and ((df_stock["close"].iloc[i - 1] < df_stock['34EMA'].iloc[i - 1]) and (df_stock["close"].iloc[i - 1] < df_stock['55EMA'].iloc[i - 1])) and (df_stock["low"].iloc[i] < df_stock["low"].iloc[i - 1]) and trade_happen[message['symbol']] == 0 and not(str(message['symbol']) in str(dfdatabase['Symbol'])):
                    nu_of_lot = int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'no_of_lot'].iloc[0])
                    Quantity = int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'Quantity'].iloc[0])
                    
                    df1 = pd.DataFrame([[message['symbol'], datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y %H:%M:%S"), "SHORT", message['ltp'], nu_of_lot, Quantity]], columns=["Symbol", "Date", "Signal", "Price", "Lots", "Qty"])
                ...
                elif (df_stock["RSI"].iloc[i] < 30 or ((df_stock["34EMA"].iloc[i - 1] > df_stock["55EMA"].iloc[i - 1]) and (df_stock["34EMA"].iloc[i] < df_stock["55EMA"].iloc[i]))) and trade_happen[message['symbol']] == 1 and str(dfdatabase.loc[dfdatabase['Symbol'] == str(message['symbol']), 'Signal'].iloc[0]) == "LONG":
                    nu_of_lot = int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'no_of_lot'].iloc[0])
                    Quantity = int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'Quantity'].iloc[0])
                    
                    df1 = pd.DataFrame([[message['symbol'], datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y %H:%M:%S"), "LONGEXIT", message['ltp'], nu_of_lot, Quantity]], columns=["Symbol", "Date", "Signal", "Price", "Lots", "Qty"])

                    trade_happen[message['symbol']] = 0
                    
                    df_signal = pd.concat([df_signal, df1])
                    df_signal.to_csv('C://upstox_oauth//media//Upstox_Tradesheet//'+datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y")+'_signal_Upstox.csv', index=False)
                    
                    #df1_database = pd.DataFrame([[message['symbol'], datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y %H:%M:%S"), "LONGEXIT", message['ltp'], nu_of_lot, Quantity]], columns=["Symbol", "Date", "Signal", "Price", "Lots", "Qty"])
                ...
                elif (df_stock["RSI"].iloc[i] > 70 or ((df_stock["34EMA"].iloc[i - 1] < df_stock["55EMA"].iloc[i - 1]) and (df_stock["34EMA"].iloc[i] > df_stock["55EMA"].iloc[i]))) and trade_happen[message['symbol']] == 1 and str(dfdatabase.loc[dfdatabase['Symbol'] == str(message['symbol']), 'Signal'].iloc[0]) == "SHORT":
                    nu_of_lot = int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'no_of_lot'].iloc[0])
                    Quantity = int(stocklist.loc[stocklist['ContractSymbol'] == str(message['symbol']), 'Quantity'].iloc[0])
                    
                    df1 = pd.DataFrame([[message['symbol'], datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y %H:%M:%S"), "SHORTEXIT", message['ltp'], nu_of_lot, Quantity]], columns=["Symbol", "Date", "Signal", "Price", "Lots", "Qty"])
                    trade_happen[message['symbol']] = 0
                    
                    df_signal = pd.concat([df_signal, df1])
                    df_signal.to_csv('C://upstox_oauth//media//Upstox_Tradesheet//'+datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y")+'_signal_Upstox.csv', index=False)
                    
                    #df1_database = pd.DataFrame([[message['symbol'], datetime.now(pytz.timezone(tz)).strftime("%d-%m-%y %H:%M:%S"), "SHORTEXIT", message['ltp'], nu_of_lot, Quantity]], columns=["Symbol", "Date", "Signal", "Price", "Lots", "Qty"])
                    ...
                    if(len(df_new)!=0 and  (str(stocks_pos['Symbol'].iloc[i]) in str(df_new['Symbol']))):
                        if(str(stocks_pos['Signal'].iloc[i])=="LONG"):
                            df_bar = pd.DataFrame([[stocks_pos['Symbol'].iloc[i],stocks_pos['Date'].iloc[i],stocks_pos['Signal'].iloc[i],\
                                                   stocks_pos['Price'].iloc[i],stocks_pos['Lots'].iloc[i],stocks_pos['Qty'].iloc[i],\
...
def scrip_master_updation(request):
    ...
    main_stocklist = pd.read_csv("C://upstox_oauth//media//Upstox_stocklist.csv")
    stocks_name = []
    df_stocks = pd.DataFrame()
    stocks_name = str(stocklist123["ContractSymbol"].iloc[0]).split(',')
    for i in range(len(stocks_name)):
        df1 = pd.DataFrame([[str(stocks_name[i]), str(params.split('*&*')[0]), main_stocklist.loc[main_stocklist['ContractSymbol'] == str(stocks_name[i]), 'ExchangeSymbol'].iloc[0]]], columns=["ContractSymbol", "name_of_marketwatch", "ExchangeSymbol"])
        ...
        if len(df_marketwatch) == 0:
            ...
            for k in range(0, len(df_marketwatch)):
                ...
                try: 
                   ...
                   value4 = int(df_pos.loc[df_pos["ContractSymbol"] == str(df_marketwatch['ContractSymbol'].iloc[k]), 'Timeframe'].iloc[0])
                   value5 = int(df_pos.loc[df_pos["ContractSymbol"] == str(df_marketwatch['ContractSymbol'].iloc[k]), 'no_of_lot'].iloc[0])
                   value6 = int(df_pos.loc[df_pos["ContractSymbol"] == str(df_marketwatch['ContractSymbol'].iloc[k]), 'Quantity'].iloc[0])
