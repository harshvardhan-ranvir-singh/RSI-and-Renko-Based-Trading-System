...
# -*- coding: utf-8 -*-
"""
OrderPlacement(TransactionType.Buy, Close)
OrderPlacement(TransactionType.Sell, Close)
...
u = Upstox('aocg0mAF6R2uotb5Mae4Q1PyBBlznq6i7EakN5Dc', '4686c9fcc2769ddca5792e7c45f08a4e36e65eaf')
d = u.get_master_contract('MCX_FO') # get contracts for MCX FO

OptionExchange = "MCX_FO"
OptionContract = "CRUDEOIL19SEPFUT"
val = u.get_instrument_by_symbol(OptionExchange, OptionContract)
df =pd.DataFrame(columns = ['Time', 'Close', 'Trend', 'Trade', 'Brick', 'PivotPrice'])
...
def event_handler_quote_update(message):
    global LTprice, bar_written, stock_data, time, flag, pivot_Price, Close, trend, trade, box_size, brick, n_multiplies_box_size 
    time=datetime.now(pytz.timezone(tz)).strftime("%H:%M:%S") 
    LTprice = message['ltp']
    print("LTprice = {}".format(LTprice))
    #print("Flag = {}".format(flag))
    if (time >= '15:30:00' and time <= '23:30:00'):
        if(time >='15:31:00'):
    ...
    flag = 1        #event_handler
                
        
def OrderPlacement(side, price):
	u.place_order(
